...
function RowForm({
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-3">
        <select
          value={form.ride_mode}
          onChange={(e) => onChange("ride_mode", e.target.value)}
          className="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 bg-white capitalize"
        >
          {RIDE_MODES.map((m) => (
      ...
      <td className="px-4 py-3">
        ...
        >
          <span
            className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform ${form.is_active ? "translate-x-4" : "translate-x-1"}`}
          />
        </button>
      </td>
      {/* Actions */}
...
export default function PricingManagement() {
  ...
  const ruleKeySet = new Set(
    rules.map(
      (rule) =>
        `${normalizeRuleValue(rule.car_type)}::${normalizeRuleValue(rule.ride_mode)}`,
    ),
  );
  ...
  const handleNewSave = async () => {
    ...
    const carType = normalizeRuleValue(newForm.car_type);
    const rideMode = normalizeRuleValue(newForm.ride_mode);
  ...
  const handleEditSave = async () => {
    const carType = normalizeRuleValue(editForm.car_type);
    const rideMode = normalizeRuleValue(editForm.ride_mode);
    const duplicateKey = `${carType}::${rideMode}`;
              ...
              {rules.map((rule) =>
                editId === rule.id ? (
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <span className="font-semibold text-slate-800">
                        ${parseFloat(rule.per_km_rate).toFixed(2)}
